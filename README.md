...
And then run this command to publish context configuration file:

    $ php artisan vendor:publish --provider="Cupona\Providers\ContextServiceProvider" --tag="config"

After you have your configuration published you should add some stuff to `config/app.php`:
...
And last but not least you *need* to add the context middleware to the `$routeMiddleware` array in your `App/Http/Kernel.php` file:
